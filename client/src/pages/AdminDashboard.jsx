...
export default function AdminDashboard() {
  ...
  const fetchData = async () => {
    const news = await fetch("http://localhost:5000/api/news").then(res => res.json());
    const events = await fetch("http://localhost:5000/api/events").then(res => res.json());
    const media = await fetch("http://localhost:5000/api/media").then(res => res.json());
  ...
  const handleSubmit = async () => {
    ...
    await fetch(`http://localhost:5000/api/${activeTab}`, {
      method: "POST",
      body: formData,
    });
  ...
  const handleUpdate = async () => {
    ...
    await fetch(`http://localhost:5000/api/${activeTab}/${currentItem._id}`, {
      method: "PUT",
      body: formData,
    });
  ...
  const handleDelete = async (id) => {
    await fetch(`http://localhost:5000/api/${activeTab}/${id}`, {
      method: "DELETE",
    });
    fetchData();
  };
  ...
  const fetchContacts = async () => {
    try {
      const { data } = await axios.get("http://localhost:5000/api/contact/get-contact");
      setContacts(data);
      setLoading(false);
    } catch (err) {
      console.error(err);
  ...
  const fetchSubscribers = async () => {
    try {
      const { data } = await axios.get("http://localhost:5000/api/newsletter/all");
      setSubscribers(data);
